...
function AñadirEvento({ id, onAgregarEvento, tipo, isGlobal }) {
    ...
    const onSubmit = async (data) => {
        ...
        const examenValue =
            data.Tipo === "Clase" || data.Tipo === "Clases"
                ? 0
                : data.Tipo === "Examen" || data.Tipo === "Examenes"
                ? 1
                : null;
        ...
        try {
            ...
            onAgregarEvento(response.data);
            console.log("Evento añadido correctamente:", response.data);
            // Se lo decimos al usuario
        } catch (error) {
            console.error("Error añadiendo el evento:", error.message);
        }
    };
            ...
            >
                <Form>
                    ...
                    <Field as="select" name="Tipo">
                        {tipo === "Clases" ? (
                            <>
                                <option value="Clase" label="Clase" />
